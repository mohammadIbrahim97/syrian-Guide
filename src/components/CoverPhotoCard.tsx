...
const GRADIENT = 'linear-gradient(135deg, var(--brand-indigo), var(--brand-coral))';

export default function CoverPhotoCard({ currentCover }: { currentCover: string | null }) {
  ...
  return (
    <section style={{ backgroundColor: 'white', borderRadius: '16px', padding: '32px', boxShadow: '0 4px 24px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.06)', marginBottom: '32px' }}>
      <h2 style={{ fontSize: '22px', fontWeight: 700, margin: '0 0 6px 0' }}>Cover image</h2>
      <p style={{ fontSize: '14px', color: 'var(--neutral-gray)', margin: '0 0 24px 0' }}>
        ...
        best. Without one, the brand gradient is shown. JPEG, PNG, or WebP, up to 7&nbsp;MB.
      </p>

      <div
        style={{
          width: '100%', aspectRatio: '3 / 1', borderRadius: '12px', overflow: 'hidden',
          border: '1px solid rgba(0,0,0,0.06)', marginBottom: '16px',
          background: shownCover ? undefined : GRADIENT,
        }}
      >
        {shownCover && (
